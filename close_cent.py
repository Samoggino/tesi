...
# Estrai gli attributi ClosenessCentrality, Degree e Alias dai nodi
degree_values = []
closeness_values = []
always_labels = []
...
for _, data in G_largest.nodes(data=True):
    degree = data['Degree']
    closeness = data['ClosenessCentrality']
...
# Calcola la correlazione tra Degree e Closeness Centrality
correlation = np.corrcoef(degree_values, closeness_values)[0, 1]
...
fig.add_trace(go.Scatter(
    ...
    text=always_labels,  # Mostra etichetta solo per i nodi con degree > 950
    textposition="top center",  # Posiziona il testo sopra i punti
))

# Aggiungi titolo e etichette agli assi
fig.update_layout(
    title=f'Degree by Closeness Centrality',
    xaxis_title='Degree',
...
print(f"Correlazione tra Degree e Closeness Centrality: {correlation}")
